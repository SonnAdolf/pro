...
CPro::CPro(float writing_pro, float art_learning_pro, float reading_pro, float computer_learning_pro, std::wstring note)
{
	...
	swprintf(buf, L"%.2f", this->m_computer_learning_pro);
	this->m_computer_learning_pro_str = buf;
	swprintf(buf, L"%.2f", this->m_total_pro);
	this->m_total_pro_str = buf;

}

CPro::CPro(std::wstring writing_pro, std::wstring art_learning_pro, std::wstring reading_pro, std::wstring computer_pro, std::wstring note)
{
	this->m_art_learning_pro_str = art_learning_pro;
	this->m_reading_pro_str = reading_pro;
	this->m_writing_pro_str = writing_pro;
	this->m_computer_learning_pro_str = computer_pro;
	...
	this->m_computer_learning_pro = static_cast<float>(_wtof(m_computer_learning_pro_str.c_str()));
...
float CPro::get_computer_learning_pro()
{
...
std::wstring CPro::get_computer_learning_pro_str()
{
	return m_computer_learning_pro_str;
}
